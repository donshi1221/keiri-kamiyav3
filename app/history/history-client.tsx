...
export default function HistoryClient({ year, month, records, clientRecords, globalTask }: Props) {
  ...
  // 履歴の対象は先月まで（当月以降はダッシュボードの領分）
  const now = new Date()
  const isLatestHistoryMonth = year * 12 + month >= now.getFullYear() * 12 + now.getMonth()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <Button variant="outline" size="sm" onClick={() => navigate(1)} disabled={isLatestHistoryMonth}>次月 →</Button>
      </div>
      ...
      <div className="sticky top-12 z-10 bg-warning-subtle border border-warning-subtle text-warning text-sm px-4 py-2 rounded-lg md:top-14">
        過去月（閲覧専用）— 修正が必要な場合は{' '}
        <Link href={`/?year=${year}&month=${month}`} className="underline font-medium">ダッシュボードでこの月を開く</Link>
        {' '}から行えます
      </div>
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-gray-50">
                  <tr>
                    ...
                    <th className="text-center py-2 px-3 font-medium text-gray-600">送付<br /><span className="text-xs font-normal text-gray-500">15日</span></th>
                    <th className="text-center py-2 px-3 font-medium text-gray-600">入金確認<br /><span className="text-xs font-normal text-gray-500">25日</span></th>
                  </tr>
                </thead>
                <tbody>
                  {clientRecords.map((cr) => (
                    <tr key={cr.id} className="border-b last:border-0">
                      <td className="py-3 px-4">
                        ...
                        {cr.label_snapshot && (
                          <span className="ml-2 text-xs text-gray-500">{cr.label_snapshot}</span>
                        )}
                      </td>
                  ...
                  <div key={cr.id} className="px-4 py-3">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <span className="font-medium">
                        {cr.clients?.name ?? '?'}
                        {cr.label_snapshot && <span className="ml-2 text-xs text-gray-500">{cr.label_snapshot}</span>}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-1 rounded-lg bg-gray-50 py-2 text-xs text-gray-500">
                      <div className="flex flex-col items-center gap-1">
                        <span>送付<span className="ml-1 text-gray-500">15日</span></span>
                        <CheckIcon done={!!cr.invoice_sent_at} label="送付" />
                      </div>
                      <div className="flex flex-col items-center gap-1">
                        <span>入金確認<span className="ml-1 text-gray-500">25日</span></span>
                        <CheckIcon done={!!cr.payment_confirmed_at} label="入金確認" />
                      </div>
                    </div>
                  </div>
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-gray-50">
                  <tr>
                    ...
                    <th className="text-center py-2 px-3 font-medium text-gray-600">受領<br /><span className="text-xs font-normal text-gray-500">10日</span></th>
                    <th className="text-center py-2 px-3 font-medium text-gray-600">支払予約<br /><span className="text-xs font-normal text-gray-500">15日</span></th>
                    <th className="text-center py-2 px-3 font-medium text-gray-600">支払確認<br /><span className="text-xs font-normal text-gray-500">末日</span></th>
                  </tr>
                </thead>
                <tbody>
                  {records.map((r) => {
                ...
                return (
                  <div key={r.id} className="px-4 py-3">
                    ...
                    <div className="grid grid-cols-3 gap-1 rounded-lg bg-gray-50 py-2 text-xs text-gray-500">
                      <div className="flex flex-col items-center gap-1">
                        <span>受領<span className="ml-1 text-gray-500">10日</span></span>
                        <CheckIcon done={!!r.invoice_received_at} label="受領" />
                      </div>
                      <div className="flex flex-col items-center gap-1">
                        <span>支払予約<span className="ml-1 text-gray-500">15日</span></span>
                        <CheckIcon done={!!r.payment_reserved_at} label="支払予約" />
                      </div>
                      <div className="flex flex-col items-center gap-1">
                        <span>支払確認<span className="ml-1 text-gray-500">末日</span></span>
                        <CheckIcon done={!!r.contractor_paid_at} label="支払確認" />
                      </div>
                    </div>
                  </div>
