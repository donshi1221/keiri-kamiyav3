...
export default function DeliveryClient({ initialYear, initialMonth }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 集計サマリー */}
      {rows && rows.length > 0 && (
        <div className="flex flex-wrap gap-x-5 gap-y-2 rounded-lg border bg-card px-4 py-3 text-sm">
          <span className="text-muted-foreground">対象 <span className="font-semibold text-foreground">{rows.length}</span> 件</span>
          ...
          <div className="hidden overflow-x-auto rounded-lg border bg-card md:block">
            <table className="w-full text-sm">
              <thead className="border-b bg-secondary">
                <tr>
                  <th className="px-4 py-2 text-left font-medium text-muted-foreground">編集者 / クライアント</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">タブ</th>
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground">すべき</th>
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground">納品済み</th>
                  <th className="px-3 py-2 text-center font-medium text-muted-foreground">状態</th>
                  <th className="px-3 py-2 text-center font-medium text-muted-foreground">シート</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.assignmentId} className="border-b align-top last:border-0">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-muted-foreground">{r.clientName} · {r.roleName}</div>
                      {r.message && <div className="mt-1 text-xs text-muted-foreground">{r.message}</div>}
                    </td>
                    <td className="px-3 py-3 text-muted-foreground">{r.tabTitle ?? '—'}</td>
                    <td className="px-3 py-3 text-right text-muted-foreground">{r.expected ?? '—'}</td>
          ...
          <div className="divide-y rounded-lg border bg-card md:hidden">
            {rows.map((r) => (
              <div key={r.assignmentId} className="px-4 py-3">
                ...
                <div className="flex items-center gap-4 rounded-lg bg-secondary px-3 py-2 text-sm">
                  <span className="text-muted-foreground">すべき <span className="font-medium text-foreground">{r.expected ?? '—'}</span></span>
                  <span className="text-muted-foreground">納品済み <span className="font-medium text-foreground">{r.delivered ?? '—'}</span></span>
                  {r.tabTitle && <span className="ml-auto text-xs text-muted-foreground">{r.tabTitle}</span>}
                </div>
                {r.message && <div className="mt-1 text-xs text-muted-foreground">{r.message}</div>}
                {r.spreadsheetUrl && (
